...
（2026-04 追記）LLM Agent からの呼び出しでは **`intent_tags` を渡す経路を優先**し、
タグ選定に SLM を使わない。詳細は **D-8**。
...
### D-8: LLM Agent からの呼び出しでは `intent_tags` を優先し、SLM はフォールバックに限定する

（2026-04 追記）

典型的な呼び出し元は AI コーディングエージェント（Claude / Codex / Cursor 等）である。
エージェントは `aegis_get_known_tags` で承認済みタグ語彙とカタログハッシュを取得し、
`compile_context` に `intent_tags` を渡すことで、`expanded` を **決定的かつ高精度** に得られる。

この経路ではサーバー側 SLM は用いない（`intent_tags` が **省略** された場合に限り、
SLM が opt-in で有効なら `plan` からタグ推論を行う）。

CLI や非 LLM クライアントでは従来どおり `intent_tags` 省略 + SLM（任意）の組み合わせが残る。

D-2 の「SLM の正式用途は `expanded context` のみ」は維持しつつ、
**第一候補の入力は Agent 提供の `intent_tags`**、SLM は **補助的フォールバック** として位置づける。
