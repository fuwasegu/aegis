...
Phase 9: Collaborative Project-Share Authoring (ADR-018)
```
...
ADR-017 (distribution bundle/status/docs) ──→ ADR-018 (shared source parser)
ADR-018 (shared source parser) ──→ ADR-018 (share-lint/share-format) ──→ ADR-018 (share-materialize)
ADR-018 (shared source parser) ──→ ADR-018 (bootstrap source export)
ADR-018 (share-materialize/bootstrap) ──→ ADR-018 (collaborative workflow docs)
```
