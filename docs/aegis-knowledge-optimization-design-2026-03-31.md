...
```typescript
// source_refs_json の構造
interface SourceRef {
  asset_path: string;           // 元ファイルのパス
  anchor_type: 'file' | 'heading_path' | 'line_range' | 'symbol_id';
  anchor_value?: string;        // e.g., "## Authentication", "L10-L45", "class AuthMiddleware"
}
// file-level は必須、section-level は任意
// 自由文 anchor は不可 — 決定的に再解決可能な anchor のみ
```

### 2.3 sync_docs の責務分離

`source_refs_json` 導入に伴い：
- **単一 source の doc**（`source_path` のみ）: 従来通り hash sync（`sync_docs` の責務）
- **複数 source の delivery unit**（`source_refs_json` あり）: `optimization/staleness.ts` の分析対象

単一 source → 単一 doc は機械的に同期可能だが、複数 source → 1 delivery unit は optimization の問題。
